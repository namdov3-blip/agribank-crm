...
export const Admin: React.FC<AdminProps> = ({ 
  ...
  const [newUser, setNewUser] = useState<Partial<User>>({
    name: '',
    role: 'User2',
    permissions: ['dashboard', 'projects', 'transactions'], // Default perms
    password: '',
  ...
  const handleCreateUser = () => {
    ...
    const userToAdd: User = {
      ...
      // Nếu là super admin: dùng organization chọn từ dropdown, ngược lại luôn là org hiện tại
      organization: isSuperAdmin ? newUser.organization || currentUser.organization : currentUser.organization
    };
    onAddUser(userToAdd);
    ...
    setAuditLogs(prev => [...prev, {
      ...
      details: `Tạo tài khoản mới: ${userToAdd.name} (${userToAdd.role})${currentUser.organization ? ` - ${currentUser.organization}` : ''}`
    }]);

    setNewUser({ name: '', role: 'User2', permissions: ['dashboard', 'projects', 'transactions'], password: '', organization: currentUser.organization });
  };
  ...
  const handleSaveInterest = () => {
    ...
    if (parsed !== interestRate && parsed > 0) {
        // Backend saves interest history (InterestSetting) and audit log
        // App.tsx handleUpdateInterestRate reloads history from backend
    ...
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      {activeSubTab === 'users' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
           {/* User List - Table by Organization */}
              ...
              <GlassCard className="p-0 overflow-hidden border-slate-300">
                 <div className="max-h-[600px] overflow-y-auto custom-scrollbar">
                    <table className="w-full text-sm text-left">
                       <thead className="bg-slate-100 text-[10px] text-slate-600 uppercase font-bold sticky top-0 z-10">
                          <tr>
                             <th className="px-6 py-3 border-b border-slate-200">Tên người dùng</th>
                             ...
                             <th className="px-6 py-3 border-b border-slate-200">Thao tác</th>
                          </tr>
                       </thead>
                       <tbody className="divide-y divide-slate-200">
                          {users.length > 0 ? (
                             users.map(user => (
                                <tr key={user.id} className="hover:bg-blue-50/50">
                                   ...
                                   <td className="px-6 py-3">
                                      {user.organization ? (
                                         <span className="px-2 py-1 bg-blue-100 text-[10px] rounded border border-blue-200 text-blue-700 flex items-center gap-1 w-fit">
                                            <Building2 size={10} />
                                            {user.organization}
                                         </span>
                                      ) : (
                                   ...
                                   <td className="px-6 py-3">
                                      <div className="flex items-center gap-3">
                                        <button 
                                           ...
                                           className="text-xs text-slate-400 hover:text-blue-600 font-bold hover:underline flex items-center gap-1 transition-colors"
                                        >
                                        ...
                                        {user.id !== currentUser.id && (
                                          <button
                                            ...
                                            className="text-xs text-red-400 hover:text-red-600 font-bold hover:underline flex items-center gap-1 transition-colors"
                                          >
                                            <X size={12} /> Xóa
                                          </button>
                                        )}
              ...
              {isSuperAdmin && (
                <div>
                  ...
                  >
                    ...
                    <option value="Agribank Chi nhánh Đông Anh">Agribank Chi nhánh Đông Anh</option>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm animate-in fade-in zoom-in duration-200">
           <GlassCard className="w-[450px] bg-white p-6 shadow-2xl border-slate-300">
              ...
              <div className="space-y-4">
                 ...
                 {/* Organization của user được quyết định bởi backend (organizationId),
                     nên không cho phép đổi organization từ màn hình chỉnh sửa user */}
      ...
      {activeSubTab === 'interest' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           ...
           <div className="space-y-2">
              ...
              <GlassCard className="p-0 overflow-hidden border border-slate-300 rounded-lg">
                 <table className="w-full text-sm text-left">
