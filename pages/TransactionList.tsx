...
export const TransactionList: React.FC<TransactionListProps> = ({ transactions, projects, interestRate, onSelect, onPrint, searchTerm, setSearchTerm, currentUser }) => {
  ...
  const filtered = useMemo(() => {
    ...
    return transactions.filter(t => {
      ...
      // Sort by STT DS from Excel (numeric order)
      const sttA = parseInt(a.metadata?.sttDS || '999999', 10);
      const sttB = parseInt(b.metadata?.sttDS || '999999', 10);
      return sttA - sttB;
    });
  }, [transactions, projects, searchTerm]);
          ...
          <table className="w-full text-sm text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-300">
              {paginatedData.map((t, index) => {
                ...
                const totalApproved = t.compensation?.totalApproved || 0;

                if (totalApproved > 0) {
                  if (isDisbursed && t.disbursementDate) {
                    // CASE 1: Đã giải ngân -> Lãi tính đến ngày thực tế chi trả (đóng băng)
                    currentInterest = calculateInterest(totalApproved, interestRate, baseDate, new Date(t.disbursementDate));
                  } else if (!isDisbursed) {
                    // CASE 2: Chưa giải ngân (bao gồm cả PENDING & HOLD) -> Lãi tính đến hiện tại (tiếp tục chạy)
                    currentInterest = calculateInterest(totalApproved, interestRate, baseDate, new Date());
                  }
                }

                const supplementary = t.supplementaryAmount || 0;
                const totalPayout = totalApproved + currentInterest + supplementary;
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 border-r border-slate-200">
                      {/* Hiển thị trực tiếp mã giao dịch (UUID) từ backend */}
                      <span className="font-mono font-bold text-slate-800 text-xs break-all">{t.id}</span>
                    </td>
